...
class FINISH_MATCHED_GOAL_TOKEN(BaseFinishReason):
...
class FINISH_MATCHED_PATH_TOKEN(BaseFinishReason):
    def __init__(self, matched: str):
        super().__init__()
        self.matched = matched
...
class FINISH_PLAN_TAG(BaseFinishReason):
    """Detected </Plan> tag in LLM output – triggers Phase II runtime fork.

    The scheduler extracts the <Outline> block from outline_text, builds a
    Petri Net, and forks child Reqs for the enabled-transition frontier Fk.
    """

    def __init__(self, outline_text: str = ""):
        super().__init__()
        self.outline_text = outline_text  # raw text between <Outline>…</Outline>

    def to_json(self):
        return {"type": "plan_complete"}


class FINISH_STEP_TAG(BaseFinishReason):
    """A single <Step> block completed – scheduler updates Petri Net marking.

    After firing the step transition, the scheduler checks for newly enabled
    frontier transitions and either forks new children or triggers a Join.
    """
    ...
    def to_json(self):
        return {"type": "step_complete", "step_id": self.step_id}


class FINISH_MEDVERSE_SECTION(BaseFinishReason):
    """A MedVerse parallel medical section has completed generation.

    Unlike FINISH_MATCHED_GOAL_TOKEN (which triggers further forking),
    this finish reason signals that a leaf section is done and should be
    moved into zombie_reqs for evidence-weighted merging.
    """
...
class Req:
    ...
    def __init__(
        ...
        right_most_pos: int = 0,
        init_input_len: int = 0,
        start_path_idx: int = -1,
        parent_start_path_idx_stack: List[int] = [],
    ):
        ...
        self.trigger_token_id = None
        self.right_most_pos = right_most_pos
        self.init_input_len = init_input_len
        self.start_path_idx = start_path_idx
        self.parent_start_path_idx_stack = parent_start_path_idx_stack
    ...
    def init_next_round_input(
        ...
        if self.right_most_pos == 0:
            self.right_most_pos += len(self.prefix_indices)
    ...
    def check_finished(self):
        ...
        if self.right_most_pos - self.init_input_len >= self.sampling_params.max_new_tokens - 1:
        ...
        if not self.sampling_params.ignore_eos:
            ...
            if self.sampling_params.stop_token_ids:
                ...
                if matched_eos:
                    ...
                    self.trigger_token_id = last_token_id
                    return
            if self.trigger_dead:
                self.trigger_dead = False

                # Postpone the finish reason until we have more context
                if self.trigger_token_id == self.tokenizer.encode("</Goal>")[0]:
                    self.finished_reason = FINISH_MATCHED_GOAL_TOKEN(matched=self.trigger_token_id)
                elif self.trigger_token_id == self.tokenizer.encode("</Path>")[0]:
                    if len(self.output_ids) < 3:
                        self.trigger_dead = True
                    elif self.output_ids[-3] == self.tokenizer.encode("</Path>")[0]:
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    # Properties for p1
    is_zombie: bool = False # For split
    is_merge: bool = False # For merge
    zombie_reqs: List[Req] = None # old stall one
    pending_reqs: List[Req] = None # new generated reqs
    length_reqs: List[Req] = None # length reqs
    p2c_map: Dict[int, int] = None
    zombie_group: List[Tuple[str, List[Req]]] = None
    right_most_pos: List[int] = None
    ...
    def prepare_for_extend(self):
        ...
        right_most_pos = [r.right_most_pos for r in reqs]
        
        for i, r in enumerate(reqs):
            r.right_most_pos = r.right_most_pos + extend_lens[i]
    ...
    def prepare_for_decode(self):
        ...
        for r in self.reqs:
            r.right_most_pos = r.right_most_pos + 1
        self.right_most_pos = [r.right_most_pos for r in self.reqs]
    ...
    def filter_batch(
        ...
        # Split/Fork the subpath under main branch
        fork_indices = [
            i
            for i in range(len(self.reqs))
            if isinstance(self.reqs[i].finished_reason, FINISH_MATCHED_GOAL_TOKEN)
        ]
        ...
        for idx in range(len(fork_indices)):
            ...
            if dead_req.right_most_pos - dead_req.init_input_len >= dead_req.sampling_params.max_new_tokens - 30 \
                ...
                self.reqs[recover_id].trigger_dead = False
                self.reqs[recover_id].trigger_token_id = None
        
        if len(recover_indices) > 0:
            for idx in range(len(recover_indices)):
                keep_indices.append(recover_indices[idx])
                fork_indices.remove(recover_indices[idx])
        ...
        if len(fork_indices) > 0:
            for idx in range(len(fork_indices)):
                ...
                goal_idx = prefix_input.rfind("<Goal>")
                if goal_idx != -1:
                ...
                for i, e in enumerate(path_ids):
                    ...
                    req = Req(
                        ...
                        token_ids_logprob=dead_req.token_ids_logprob,
                        stream=dead_req.stream,
                        lora_path=dead_req.lora_path,
                        input_embeds=dead_req.input_embeds,
                        right_most_pos=dead_req.right_most_pos,
                        init_input_len=dead_req.init_input_len,
                        start_path_idx=start_path_idx,
                        parent_start_path_idx_stack=parent_start_path_idx_stack,
                    )
                    req.tokenizer = tokenizer
                    self.pending_reqs.append(req)
                    if parent_rid in self.p2c_map:
                        self.p2c_map[parent_rid].append(req.rid)
                    else:
                        self.p2c_map[parent_rid] = [req.rid]
            
            self.is_zombie = True
            
        
        # Process the finish subpath. If all the subpaths are finished, the main branch will be merged
        path_indices = [
            i
            for i in range(len(self.reqs))
            if self.reqs[i].finished_reason is not None and not isinstance(self.reqs[i].finished_reason, FINISH_MATCHED_GOAL_TOKEN)
        ]
        ...
        for key, value in self.p2c_map.items():
            ...
            for req in self.zombie_reqs:
                if req.rid in values:
                    values.remove(req.rid)
            if len(values) == 0:
                v = value.copy()
                zombie_group_item = (key, [])
                for rid in v:
                    for req in self.zombie_reqs:
                        if req.rid == rid:
                            zombie_group_item[1].append(req)
                            self.zombie_reqs.remove(req)
                self.zombie_group.append(zombie_group_item)
                self.is_merge = True
                removed_keys.append(key)
                
        for key in removed_keys:
            self.p2c_map.pop(key)
    ...
    def merge_batch(self, other: "ScheduleBatch", log: bool = False):
        # Penalizer orchestrator must be merged before Batch.reqs is merged. This is because
        # orchestrator.merge() depends on Batch.reqs during preparation of each penalizers, so it
        # needs to be called with pre-merged Batch.reqs.
        if other.is_zombie or other.is_merge:  
            if other.is_empty():
                for key, value in other.p2c_map.items():
                    if key in self.p2c_map:
                        self.p2c_map[key] = self.p2c_map[key]
                    else:
            ...
            assert other.p2c_map is not None
            # merge p2c_map
            for key, value in other.p2c_map.items():
                if key in self.p2c_map:
                    self.p2c_map[key] = self.p2c_map[key]
    ...
    def get_model_worker_batch(self) -> ModelWorkerBatch:
        ...
        return ModelWorkerBatch(
            ...
            right_most_pos=self.right_most_pos,
    ...
    def __str__(self):
        return (
            ...
            f"out_cache_loc={self.out_cache_loc}),"
            f"output_ids={self.output_ids}),"
            f"is_zombie={self.is_zombie}),"
            f"is_merge={self.is_merge}),"
            f"zombie_reqs={self.zombie_reqs}),"
            f"pending_reqs={self.pending_reqs}),"
            f"length_reqs={self.length_reqs}),"
            f"p2c_map={self.p2c_map}),"
        )
...
class ModelWorkerBatch:
    ...
    right_most_pos: Optional[List[int]]
