...
from sglang.srt.managers.schedule_batch import BaseFinishReason, Req, ScheduleBatch, FINISH_MATCHED_GOAL_TOKEN, FINISH_MATCHED_PATH_TOKEN, FINISH_PLAN_TAG, FINISH_STEP_TAG
...
class SchedulerOutputProcessorMixin:
    ...
    def stream_output_generation(
        ...
        for req in reqs:
            ...
            if (
                (req.finished() and not isinstance(req.finished_reason, (FINISH_MATCHED_GOAL_TOKEN, FINISH_PLAN_TAG, FINISH_STEP_TAG)))
                ...
                rids.append(req.rid)
                total_tokens.append(len(req.output_ids) + len(req.origin_input_ids))
                max_tokens.append(req.right_most_pos)
        ...
        if rids:
            if self.model_config.is_multimodal_gen:
                return
            # print(f"rids: {rids}")
            # print(f"finished_reasons: {finished_reasons}")
            # print(f"decoded_texts: {decoded_texts}")
            # print(f"decode_ids_list: {decode_ids_list}")
            # print(f"read_offsets: {read_offsets}")
