...
from utils import (
    MedVerseDataCollatorForCompletionOnlyLM,
...
class TrainingConfig:
    ...
    wandb_project: Optional[str] = field(default="MedVerse")
...
def train():
    ...
    collator = MedVerseDataCollatorForCompletionOnlyLM(
    ...
    args.do_train = True
    log_config = {**asdict(config), **asdict(args)}
    logging.info(f"Training config (effective): {log_config}")
    ...
    if trainer.accelerator.is_main_process:
        ...
        print({"eval_loss": eval_loss, "perplexity": ppl, **metrics})
        os.makedirs(args.output_dir, exist_ok=True)
        with open(os.path.join(args.output_dir, "eval_results.json"), "w") as f:
            json.dump(
                {"eval_loss": eval_loss, "perplexity": ppl, **metrics},
                f,
                ensure_ascii=False,
                indent=2,
            )
