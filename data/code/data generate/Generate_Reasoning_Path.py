...
def run_llm(model="gpt-5.1", temperature=0, messages=""):
...
def change_format(filter_reasoning_path):
    ...
    if judge==False:
        ...
        filter_reasoning_path = run_llm(model="gpt-5.1", temperature=0, messages=messages2)
        judge = validate_reasoning_chains(filter_reasoning_path)
...
jsonl_path = "./jsonl/merged_cleaned_data.jsonl"
count = 0
...
with open(jsonl_path, "r", encoding="utf-8") as f:
    for idx, line in enumerate(f):
        ...
        filter_reasoning_path = run_llm(model="gpt-5.1", temperature=0, messages=messages1)
        ...
        new_reasoning_path = run_llm(model="gpt-5.1", temperature=0.2, messages=messages3)
        ...
        new_reasoning_path = run_llm(model="gpt-5.1", temperature=0.2, messages=messages4)
        if new_reasoning_path == "**FAIL**":
            continue
        ...
        validation = run_llm(model="gpt-5.1", temperature=0, messages=messages5)
        if validation == "**FAIL**":
            continue
        # print(validation)
        if validation.lower().find('pass')!=-1:
            ...
            new_reasoning_path = run_llm(model="gpt-5.1", temperature=0, messages=repair_prompt_messages)
            if new_reasoning_path == "**FAIL**":
                continue
            new_reasoning_path = change_format(new_reasoning_path)
            if new_reasoning_path == "":
